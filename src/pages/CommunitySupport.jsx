...
function PostCard({ post }) {
  ...
  return (
    ...
    >
      ...
      <div className="border-t pt-3 text-xs text-muted-foreground font-body">
...
function NewPostForm({ user, onClose, onSuccess }) {
  ...
  const mutation = useMutation({
    mutationFn: (data) => base44.entities.CommunitySupport.create(data),
    onSuccess: () => {
...
export default function CommunitySupport() {
  ...
  const { data: posts = [] } = useQuery({
    ...
    queryFn: () => base44.entities.CommunitySupport.filter({ status: 'open' }, '-created_date', 50),
    enabled: !!user,
  });
    ...
    <div className="pt-20">
      ...
      <section className="py-20 bg-background">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          {!user ? (
            /* Locked state — visible to all, but prompts sign-in */
            <motion.div
            ...
            <>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                <div>
                  ...
                  <p className="font-body text-sm text-muted-foreground mt-1">{posts.length} open {posts.length === 1 ? 'post' : 'posts'}</p>
                </div>
                <div className="flex items-center gap-3">
                  <div className="flex bg-secondary/50 rounded-lg p-1 gap-1">
                    {[['all', 'All'], ['offer', 'Offering'], ['request', 'Seeking']].map(([val, label]) => (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-md text-xs font-body font-medium transition-colors ${filter === val ? 'bg-white shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'}`}
                      >
                        {label}
                      </button>
                    ))}
                  ...
                  <Button
                    size="sm"
                    className="font-body bg-primary hover:bg-primary/90 gap-1"
              ...
              {showForm && (
                <NewPostForm user={user} onClose={() => setShowForm(false)} onSuccess={() => setShowForm(false)} />
              )}
